...
import csv
import os
from data import bert as modeling, bert as optimization, bert as tokenization
...
class OperationProcessor(DataProcessor):
    """
...
class TitleProcessor(DataProcessor):
    """
    Operation data processor
    """
    ...
    def get_train_examples(self, data_dir):
        ...
        for index, row in df.iterrows():
            guid = "train-%d" % index
            text_a = tokenization.convert_to_unicode(row['long_title'])
            label = tokenization.convert_to_unicode(str(row['Politics']))
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
        return examples
...
class TwitterProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        for index, row in df.iterrows():
            ...
            label = tokenization.convert_to_unicode(str(row['flag']))
            examples.append(
                InputExample(guid=guid, text_a=text_a, label=label))
        return examples
