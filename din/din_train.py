...
def train():
    ...
    def load_vec():
        train_news_vec = np.load('train/train_news_vec.npy')
        dev_news_vec = np.load('dev/dev_news_vec.npy')
    ...
    train_data = load_dataset('train/copy1_5train_index.csv')
    ...
    model.fit(train_data, epochs=5, validation_data=dev_data, callbacks=cp_callback)
